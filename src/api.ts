...
export async function getImagesFromPage(category: string, page: number) {
  const url = `https://api.rule34.xxx/index.php?page=dapi&s=post&q=index&tags=${category}&pid=${page}`;
...
export async function getPagesCount(category: string) {
  const url = `https://api.rule34.xxx/index.php?page=dapi&s=post&q=index&tags=${category}&pid=0&limit=0`;
